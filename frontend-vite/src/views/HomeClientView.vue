...
const searchByOrderId = async (orderId: string) => {
  try {
    ...
    if (
      ...
      response.data.institution_id !== authStore.institutionId
    ) {
      rawOrders.value = [];
      totalOrders.value = 0;
    } else {
...
const requestedEditCount = computed(
  () =>
    rawOrders.value.filter(
      (o) => o.status === "REQUEST_TO_EDIT" || o.status === "APPROVED_EDITED",
    ).length,
);
...
<template>
  <div class="flex h-screen bg-[#F9FAFB] text-zinc-900 font-inter">
    ...
    >
      <div class="p-6 h-full flex flex-col">
        <div class="flex items-center gap-3 mb-10">
          <img
            src="@/assets/dcsa-logo.svg"
            alt="DCSA Logo"
            class="h-8 w-auto brightness-125 filter-[invert(48%)_sepia(79%)_saturate(2476%)_hue-rotate(120deg)_brightness(118%)_contrast(119%)]"
          />
        ...
        <nav class="space-y-1 flex-1">
          <a
            href="#dashboard"
            class="flex items-center gap-3 px-4 py-3 text-zinc-200 bg-zinc-800/50 rounded-lg group"
          >
        ...
        <div class="mt-auto pt-6 border-t border-zinc-800">
          <Button
            @click="handleLogout"
            variant="ghost"
            class="w-full justify-start gap-3 text-zinc-400 hover:text-red-400 hover:bg-red-400/10 transition-colors"
          >
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      <!-- Header -->
      <header
        class="h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-6 shrink-0"
      >
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                ...
                >
                  <Bell class="h-5 w-5" />
                  <span
                    v-if="requestedEditCount > 0"
                    class="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border border-white"
                  ></span>
                </Button>
              </TooltipTrigger>
              <TooltipContent>
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        <!-- Top Filters Section -->
        <div
          class="bg-white border border-zinc-100 rounded-xl p-6 shadow-sm space-y-6"
        >
          ...
          <div class="flex items-center gap-3">
            <span class="text-sm font-medium text-emerald-600 mr-2"
              >Status:</span
            >
        ...
        <!-- Table Section -->
        <div
          class="bg-white border border-zinc-100 rounded-xl shadow-sm overflow-hidden"
        >
          <!-- Loading State -->
          <div
            v-if="isInitialLoad"
          ...
          <Table v-else>
            <TableHeader class="bg-zinc-50/50">
              <TableRow class="hover:bg-transparent">
                <TableHead
                  ...
                  class="cursor-pointer hover:text-emerald-600 text-[11px] font-bold uppercase tracking-wider text-emerald-600/70 group"
                >
                ...
                <TableHead
                  class="text-[11px] font-bold uppercase tracking-wider text-emerald-600/70"
                  >Order ID</TableHead
                >
                <TableHead
                  class="text-[11px] font-bold uppercase tracking-wider text-emerald-600/70"
                ...
                >
                <TableHead
                  class="text-right text-[11px] font-bold uppercase tracking-wider text-emerald-600/70"
                  >Actions</TableHead
                >
              </TableRow>
            </TableHeader>
            <TableBody>
              <TableRow
                ...
                :key="order.order_id"
                class="group transition-colors h-16 border-b border-zinc-50"
                @click="handleEditDetails(order)"
              >
                ...
                <TableCell class="font-bold text-zinc-900"
                  >{{ order.total_portion.toLocaleString() }} pax</TableCell
                >
                <TableCell class="text-zinc-500 font-medium">{{
                  order.dropping_location_food || "-"
                }}</TableCell>
                <TableCell>
                  <span class="text-emerald-600 font-medium"
                    >{{ getStaffCount(order.staff_allocation) }} staff</span
                  >
                </TableCell>
                <TableCell>
                  <Badge
                    ...
                    class="rounded-lg px-3 py-1 font-bold shadow-sm whitespace-nowrap"
                  >
                ...
                <TableCell class="text-right">
                  <Button
                    variant="ghost"
                    size="icon"
                    class="h-8 w-8 text-zinc-400 group-hover:text-emerald-600 transition-colors"
                    @click.stop="handleEditDetails(order)"
                  >
          ...
          <div
            ...
            class="px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-zinc-50 bg-zinc-50/20"
          >
            <div
              class="flex items-center gap-4 text-xs font-medium text-emerald-600/70"
            >
              ...
              <div>
                {{
                  filteredOrders.length > 0
                    ...
                    : 0
                }}-{{ (currentPage - 1) * pageSize + filteredOrders.length }} of
                {{ totalOrders }}
              </div>
            </div>
