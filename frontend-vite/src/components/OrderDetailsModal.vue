...
import { Save, Plus, Trash2, X } from 'lucide-vue-next'
...
const handleSave = async () => {
  ...
  try {
    ...
    const calculatedTotal = Object.values(staffAllocation).reduce((sum, item) => sum + item.total, 0)
...
<template>
  <Dialog :open="open" @update:open="(val) => emit('update:open', val)">
    <DialogContent class="max-w-[800px] p-0 overflow-hidden bg-white" style="height: 85vh;">

      <!-- Header Area - Fixed -->
      <div class="bg-zinc-50 border-b border-zinc-100 p-6 flex items-center justify-between shrink-0">
        <div>
        ...
        <div class="text-right">
           <div class="text-sm font-medium text-zinc-900">{{ order?.institution_name || '-' }}</div>
           <div class="text-xs text-zinc-500">{{ order?.order_date || '-' }}</div>
        </div>
      </div>

      <!-- Body - Scrollable -->
      <div class="p-6 space-y-6 overflow-y-auto" style="max-height: calc(85vh - 160px);">

        <!-- Section: Staff Allocation -->
        <div class="space-y-4">
          <div class="flex items-center justify-between border-b pb-2">
             <h3 class="font-bold text-zinc-700 text-sm uppercase tracking-wide">Staff Allocation</h3>
             <Button
               variant="ghost"
               size="sm"
          ...
          <div class="border rounded-lg overflow-hidden bg-white">
            <Table>
              ...
              <TableBody>
                 <TableRow v-for="(item, idx) in staffAllocations" :key="idx" class="group">
                   <TableCell>
                      <Input
          ...
          <div class="grid grid-cols-1 gap-4">
            ...
            <div v-for="category in menuCategories" :key="category.key" class="border rounded-lg bg-white overflow-hidden">
              ...
              <div class="divide-y divide-zinc-50">
                <div
                  v-for="(item, idx) in menuDetails[category.key as keyof MenuDetails]"
                  :key="idx"
                ...
                <div v-if="!menuDetails[category.key as keyof MenuDetails]?.length" class="p-4 text-center text-zinc-400 text-sm">
      ...
      <div class="p-6 bg-zinc-50 border-t border-zinc-100 shrink-0">
        <div class="w-full flex items-center justify-between">
           ...
           <Button class="bg-emerald-600 hover:bg-emerald-700 text-white gap-2" :disabled="isSaving" @click="handleSave">
              <Save class="h-4 w-4" />
